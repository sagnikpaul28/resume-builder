<template>
    <div class="editor">
        ...
        <div class="row">
            <draggable class="list-group" :list="data.languages" group="people"> 
                <transition-group>
                    <div class="col col-3" v-for="(language, index) in data.languages" :key="index">
        ...
        <div class="row add-new">
            ...
            <input type="text" v-model="newFields.languages" />
            <button type="btn" class="btn" @click="onAddLanguages()">Add</button>
        </div>
    ...
    methods: {
        onAddLanguages() {
            this.$store.dispatch('addDetails', {
                type: 'languages',
                value: {
                    name: this.newFields.languages
                }
            });
            this.newFields.languages = '';
        },
        deleteItem(index) {
        ...
        onButtonSubmit() {
            fetch('http://localhost:4000/save-details', {
                ...
                body: JSON.stringify({
                    ...
                    "field": "languages",
                    "value": JSON.stringify(this.data.languages)
                }),
                headers: {
                    'Content-Type':'application/json',
                }
            }).then(() => {
                this.$store.dispatch('afterLayoutChooseNextState', 'languages').then(() => {
