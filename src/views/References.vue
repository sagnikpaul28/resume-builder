<template>
    <div class="editor">
        ...
        <draggable class="list-group" :list="references" group="people"> 
            <transition-group>
                <div class="row" v-for="(reference, index) in references" :key="index">
                    ...
                    <div class="popup" :class="{'show': reference.showModal}">
                        <div class="row">
                            <div class="col">
                            ...
                            <div class="col">
                                <div class="input-containers">
                                    ...
                                    <input type="text" v-model="reference.position" />
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col">
                                <div class="input-containers">
                                    <label>Email:</label>
                                    <input type="text" v-model="reference.email" />
                                </div>
                            </div>
                            <div class="col">
                                <div class="input-containers">
                                    <label>Number:</label>
                                    <input type="text" v-model="reference.number" />
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col">
                                <div class="input-containers">
                                    ...
                                    <textarea rows="4" v-model="reference.description" />
                                </div>
                            </div>
                        </div>
        ...
        <div class="row footer">
            <button class="btn add-new" @click="toggleNewItemModal()"><span>+</span> Add References</button>
        ...
        <div class="popup" :class="{'show': showNewItemAddModal}">
            <div class="row">
                ...
                <div class="col">
                    <div class="input-containers">
                        <label>Position:</label>
                        <input type="text" v-model="newFields.position" />
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col">
                    <div class="input-containers">
                        <label>Email:</label>
                        <input type="text" v-model="newFields.email" />
                    </div>
                </div>
                <div class="col">
                    <div class="input-containers">
                        <label>Number:</label>
                        <input type="text" v-model="newFields.number" />
                    </div>
                </div>
            </div>
            <div class="row">
...
export default {
    ...
    computed: {
        references() {
            return this.$store.state.data.references
        },
        newFields() {
            return this.$store.state.newFields.references
        }
    },
    methods: {
        openModal(index) {
        ...
        onButtonSubmit() {
            fetch('http://localhost:4000/save-details', {
                ...
                body: JSON.stringify({
                    ...
                    "field": "references",
                    "value": JSON.stringify(this.references)
                }),
                headers: {
                    'Content-Type':'application/json',
                }
            }).then(() => {
                this.$store.dispatch('afterLayoutChooseNextState', 'references').then(() => {
